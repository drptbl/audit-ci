...
const modifyData = async (data, showUnique) => {
  const auditAdvisories = data.filter(
    vulnerability => vulnerability.type === 'auditAdvisory'
  );
  const reportDate = new Date();
  const summaryData = data.pop().data;
  const summary = {
    ...summaryData,
    vulnerabilities: Object.values(summaryData.vulnerabilities).reduce(
      (sum, next) => sum + next,
      0
    ),
  };
  ...
  vulnerabilities.sort(
    (left, right) =>
      severitySortPriority.indexOf(left.severity) -
      severitySortPriority.indexOf(right.severity)
  );
...
Handlebars.registerHelper(
  'severityClass',
  severity => bootstrapClassSeverityMap[severity]
);
